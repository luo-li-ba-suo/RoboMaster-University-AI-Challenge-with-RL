...
namespace hitcrt
{
	...
	vector<uint> FindNeighbor(MapSearchNode& Node, vector<MapSearchNode>& Nodes, vector<vector<MapSearchNode>>& Obstacles, bool start_flag);//寻找起始点和终止点的邻接节点
	bool Compare(const vector<float>& a, const vector<float>& b);//排序比较函数
	bool IsBlocked(MapSearchNode& NodeA, MapSearchNode& NodeB, vector<vector<MapSearchNode>>& Obstacles, bool start_flag);//判断两节点是否被阻挡
